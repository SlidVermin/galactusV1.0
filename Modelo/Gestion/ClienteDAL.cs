...
namespace Galactus.Modelo.Gestion
{
    class ClienteDAL
    {
        public static Cliente guardarCliente(Cliente cliente) {
            try
            {
                using (SqlCommand sentencia = new SqlCommand())
                {
                    ...
                    sentencia.Parameters.Add(new SqlParameter("@codigo", SqlDbType.Int)).Value = cliente.codigo;
                    sentencia.Parameters.Add(new SqlParameter("@codigoTercero", SqlDbType.Int)).Value = cliente.codigoTercero;
                    sentencia.Parameters.Add(new SqlParameter("@codigoRegimen", SqlDbType.NVarChar)).Value = cliente.codigoRegimen;
                    sentencia.Parameters.Add(new SqlParameter("@codigoUbicacion", SqlDbType.NVarChar)).Value = cliente.codigoUbicacion;
                    sentencia.Parameters.Add(new SqlParameter("@codigoFormaPago", SqlDbType.NVarChar)).Value = cliente.codigoFormaPago;
                    ...
                    cliente.codigo = (int)sentencia.ExecuteScalar();
                }
            }
            catch (Exception ex)
            {
        ...
        public static Boolean anularCliente(int codigo) {
